...
- Use the approved modular monolith, monorepo, Windows development, GitHub automation, Render development services, Google Maps integration, and documented production approach consistently.
...
- Keep local and CI commands equivalent and treat CI as the authoritative gate.
...
- Use Keycloak Authorization Code Flow with PKCE through `flutter_appauth`; never embed a privileged client secret.
- Store only necessary authentication material in platform secure storage and remove it on logout, revocation, or account disablement.
- Handle token expiry, renewal failure, cancelled login, lost connectivity, and forced reauthentication explicitly.
...
- Use `react-oidc-context` with PKCE and approved Keycloak redirect and logout URLs.
- Do not store privileged secrets or unnecessarily persist access tokens in browser storage.
- Handle renewal, expiry, multi-tab logout, cancelled login, and unauthorised routes predictably.
...
- Validate behaviour behind Nginx and Render proxies, including WebSocket upgrades and client IP handling.
...
- Generate Dart and TypeScript clients through one reproducible command and fail CI when generated output drifts.
...
- Isolate Google Maps, Africa’s Talking, Firebase, and Keycloak behind focused adapters owned by the relevant module.
...
- Define degraded behaviour for unavailable maps, SMS, push, identity, GitHub, or Render services.
...
### Keycloak Configuration and Integration

- Keep realms, clients, roles, redirect URIs, token settings, and exports environment-specific and version-controlled where safe.
- Restrict Keycloak administration to authorised operators and never expose its administrative credentials to public clients.
- Test configuration portability and restore rather than relying on undocumented dashboard changes.

### OAuth 2.0 and OpenID Connect Security

- Use Authorization Code Flow with PKCE for mobile and browser clients and never embed a client secret in either.
- Validate issuer, audience, signature, algorithm, expiry, and required claims on every API token.
- Reject tokens intended for another environment or client and handle revocation and renewal safely.
...
### TypeORM and SQL Usage Boundaries

- Use TypeORM for ordinary persistence and reviewed parameterised SQL for complex spatial, reporting, locking, or performance-sensitive work.
...
- Store secrets only in approved local ignored files, GitHub environments, Render settings, or SOPS/age-encrypted files where required.
...
- Apply the same rules to Pino, browser telemetry, mobile diagnostics, traces, CI logs, and provider errors.
...
- Run PostgreSQL/PostGIS, Valkey 9, and Keycloak integration tests with pinned containers on GitHub-hosted Linux runners.
- Apply real migrations and verify spatial queries, queue behaviour, token validation, and transaction boundaries.
- Keep tests isolated, deterministic, time-bounded, and independent of shared Render development data.

### API Contract and Generated-Client Testing

- Fail CI when OpenAPI output or generated Dart and TypeScript clients differ from committed reviewed output.
...
- Separate lint, unit, integration, security, build, and deploy jobs with explicit dependencies and timeouts.
- Use Windows runners only for material Windows checks and Ubuntu runners for Linux and container tests.
- Pin reviewed actions, restrict permissions, control concurrency, validate caching, and avoid exposing secrets to untrusted pull requests.
...
- Build containers remotely in GitHub Actions or Render; do not require Docker Desktop locally.
...
- Pin workflow actions and base images and review provenance and licence changes.

### Deployment Automation and Approval Controls

- Deploy reviewed commits through GitHub Actions and approved Render or production automation, never undocumented manual steps.
...
## 17. Windows and Render Development
...
- Test cross-platform Task commands in Windows and Linux CI when both are claimed.
...
- Run Linux-specific containers, Valkey 9 integration, and other container checks in GitHub Actions or Render.
- Do not expose workstation services publicly to bridge remote dependencies.

### Render Service Architecture

- Keep Valkey 9 private and pinned, the API externally reachable only where required, and BullMQ workers non-public.
- Place dependent services in compatible regions and configure health, persistence, authentication, capacity, and recovery explicitly.
- Label PostgreSQL/PostGIS, Keycloak, and monitoring hosting as unresolved until formally approved.

### Render Infrastructure as Code

- Define supported Render resources and non-secret configuration in reviewed `render.yaml` and use the Render API for controlled automation.
- Keep secret values in Render settings or approved GitHub secrets and detect dashboard drift.
- Review infrastructure changes with the application changes that depend on them.

### Render Networking and Service Exposure

- Use private networking for Valkey and internal dependencies and TLS public endpoints only for services clients must reach.
- Never publish databases, Valkey, worker control ports, or administrative interfaces without an approved secured requirement.
- Document trust boundaries, ingress, egress, authentication, and allowed callers.

### Render Cost and Resource Governance

- Select the smallest reliable paid resources, persistent disks, and retention that meet approved development needs.
- Track bandwidth, builds, storage, compute, and idle services with an assigned cost owner and budget.
- Suspend or remove unused resources through approved recoverable procedures.

### Development Connectivity and Outage Handling

- Design the workflow for temporary loss of internet, GitHub, Render, Google Maps, Firebase, or Africa’s Talking.
...
- Use Restic for approved configuration, Keycloak exports, and operational files not held in PostgreSQL.
...
- Track supported versions of operating systems, runtimes, databases, Keycloak, containers, dependencies, and tools.
...
- Maintain executable runbooks for deployment, rollback, queues, notifications, Google quotas, databases, certificates, Keycloak, backups, and dependency outages.
...
- Make documentation builds reproducible and validate links, navigation, and rendering in CI.
...
- Identify every change that can increase Google Maps, Render, Africa’s Talking, GitHub, Firebase Test Lab, infrastructure, or distribution costs.
...
- Use approved organisational ownership for repositories, Render, Google Cloud, Firebase, Africa’s Talking, domains, billing, and recovery methods.
