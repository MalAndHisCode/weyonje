...
- Repeating Client registration for the same unverified phone resumes the original pending Client without overwriting profile data or creating another user. If the previous request was interrupted, wait for the cooldown before submitting again. Existing verified identifiers still require sign-in. Provider retries now resume only an unverified, inactive, login-disabled PENDING profile without replacing business/contact fields. Verified/restricted accounts cannot recover by re-registering.
...
- If commit succeeds but the HTTP response is lost, the code stays consumed. Return to Client Sign In and request a fresh code; Provider users use Service Provider phone sign-in. No raw session or replayable OTP response is persisted server-side to simulate exactly-once delivery.
...
## Service Provider Phone Authentication and Temporary Approval — 2026-09-11

Implemented locally; not deployed. The registered account phone number (not contact-person phone) is the Provider sign-in identifier. Email remains required contact information. POST /v1/auth/provider-code/request, /resend and /verify reuse protected phone normalization, OTP delivery and native sessions; unknown/wrong-actor/ineligible phones share safe guidance and an explicit registration action. The Client-only REGISTRATION_REQUIRED outcome is unchanged. The Provider request uses separate HMAC phone/IP namespaces and PROVIDER_REQUEST retry metadata with existing limits. Registration, Client and Provider codes cannot substitute for one another.

At verification, a Provider user row lock protects rechecking actor, the challenged phone lookup, verified phone and loginEnabled in the OTP/session transaction. Pending/rejected/inactive accounts retain restricted status routing where authentication is allowed; approved/active/login-enabled remains required for work. Sign-in and refresh never approve or reactivate accounts.

New Provider registrations store passwordHash=null and retain all required business/contact validation. Provider password login, recovery issuance/resend and recovery completion are disallowed; historical hashes and sessions are not bulk-cleared. KCCA password login/recovery and authenticated email verification remain available. An interrupted unverified registration can be resubmitted after cooldown; it resumes the original stored profile and never accepts unauthenticated replacement details. If credentials were lost after successful verification, request a new sign-in code rather than replaying the consumed registration code.

SERVICE_PROVIDER_AUTO_APPROVAL_ENABLED defaults to false and accepts only true/false. The authorized ignored local .env explicitly enables it for this task. While enabled, registrations completing phone verification atomically receive APPROVED, activation, login, one WSP number, system-attributed decision/status/audit records, truthful approval notifications/outbox and a session. No pending-review notification is emitted. Disabled mode retains verified-but-pending manual review. Existing verified pending, rejected, inactive or disabled accounts are not auto-approved. Setting false changes future completions only.

The exact red KCCA Approval Required heading remains by user request with its paragraph removed. The temporary policy explicitly bypasses the Business Process Specification's manual review step; it does not claim a human KCCA review or remove KCCA administration controls.

Rollout order: review and apply forward migration 20260911140000_provider_phone_auth_auto_approval, deploy the updated API/contracts, explicitly configure the policy on the target server, then distribute the updated mobile APK. The migration adds an enum value and narrow provenance/nullability changes; assess table locks when scheduling it. Historical manual records default to KCCA_MANUAL. No shared migration or deployed setting was changed here. Test migrations against an isolated PostgreSQL instance first using TEST_DATABASE_URL and TEST_DIRECT_URL.

Older Provider mobile builds cannot use password login after this API change and their registration password property is rejected by allow-list validation. New mobile phone sign-in requires the updated API. Plan a coordinated API-first rollout with an app-update/support notice; this is not transparent compatibility. Existing verified Providers do not need re-registration. Mobile config/auth.local.json currently points at Railway, so the debug APK is not proof that the local policy is active on its target. Do not roll back to the old password-accepting API to disable approval: set the policy false on the new API instead.

Validation is synthetic/local unless explicitly stated otherwise. Isolated database tests, physical SMS receipt/retrieval and deployed behavior remain unverified. No live SMS or device installation is authorized by this implementation.
