...
Historical Railway inspection was read-only (current deployment evidence is below): weyonje-api was Sleeping on the Trial plan, in an environment named production. Its settings warn that the configured ams region is invalid and blocks deployments. No region, variable, plan, or deployment was changed. Railway's [static outbound IP documentation](https://docs.railway.com/networking/static-outbound-ips) requires Pro and a redeploy. Consequently, no unrestricted server key was created as a workaround.
...
3. Approve the production-named service's region correction and API deployment explicitly. Server Maps credentials remain deferred and are not required for this coordinate-backed Client flow; local configuration does not update Railway. The later c153536 deployment includes the profile endpoint/Client DTO; investigate the unresolved POST failure below before claiming submission works.
...
5. Connect an authorized Android device and verify native tiles/attribution, both permission modes and settings recovery, actual GPS, pan/pinch/tap selection, cancellation, offline map feedback, and warm display-off/on. Test process death separately; durable draft restoration is not implemented.
...
## Submission correction — 2026-09-11

The local failure was reproduced at strict HTTP enum validation: the previous Client form sent scheduleMode: ASAP instead of AS_SOON_AS_POSSIBLE. It now sends the contract value. The corrected payload reaches real service creation and detail mapping in HTTP tests with isolated persistence fakes; malformed inputs still fail. The exception filter provides safe owned messages for known validation fields using the existing code/message/requestId response. No contract/schema/configuration change is required. This evidence does not establish the version or health of the installed APK or deployed Railway API. See CURRENT_SYSTEM_STATE.md for final tests/build and remaining device/rollout limitations.


## Later Railway POST failure and local presentation update — 2026-09-11

Read-only inspection confirms deployment a77cc5c2-87f0-44ec-b2f4-843bc23c8c44 links to main commit c1535365295e249e42099848f6247d5e037e27ce. Its logs contain request c7882a2a-cae6-4aac-9951-ece06f2ad7c6 and a subsequent POST failure; neither records the cause or commit outcome. This supersedes earlier rollout-pending assumptions. The service was Sleeping; no setting or deployment was changed. The old ASAP validation defect does not explain this unexpected-exception event.

Local presentation now uses read-only profile text boxes, places Select Location on Map above the No preview and always places Check My Requests below Submit Request. List round trips preserve the draft, key and uncertainty lock. One concise uncertain-status alert replaces the redundant warning.

The local diagnostic build adds safe owned failure stages, categories and recognized Prisma codes to request_failed, retaining request IDs and template routes without personal data, SQL or raw exceptions. It does not claim to fix the unknown production cause. Deploying diagnostics requires separate authorization. Correlate subsequent naturally occurring logs; verify deployed migration compatibility without mutating production. Run the opt-in service creation/rollback/post-commit-replay PostgreSQL test only with approved isolated TEST_DATABASE_URL and TEST_DIRECT_URL. Neither is currently available; HTTP persistence fakes do not prove database behavior. No environment variable, schema, dependency, DTO or contract changed. Current system state records final validation and APK evidence.

## Railway schema repair — 2026-09-11

This supersedes the earlier unknown-cause/migration status above. After the user's diagnostic deployment, request `e01efd3a-5c2f-42aa-9f28-b57221abc5cb` failed at `outbox_write` with Prisma `P2022`. Read-only inspection found four missing outbox lease/dead-letter columns and three pending migrations. At the user's request, `pnpm --filter @weyonje/api migration:deploy` was run successfully in the Railway service console, applying reliable-delivery, account-recovery and KCCA-history migrations. Follow-up migration status reports the database up to date; all six stored migration checksums match repository SQL and the missing columns now exist. No deployment hook or application configuration was changed. Retry the existing unchanged Client draft to verify end-to-end submission; migration success alone does not establish that result. See CURRENT_SYSTEM_STATE.md revision 9.3 for evidence.
