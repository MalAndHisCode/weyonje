...
- Clients create ASAP requests, view saved details/maps/history, edit or withdraw their own unaccepted pending mobile-origin requests, track journeys, and submit 1–5 collection feedback. Editing reuses the form, preserves historical scheduling and saved CURRENT coordinates, and uses server-checked updatedAt plus a stable operation key. Withdrawal retains Cancelled history; linked Call Centre requests are not editable. Unknown outcomes keep the command/draft for unchanged retry. Drafts are in memory, not restored after process death.
...
Client management also requires PUT `/v1/client/requests/:id` and POST `/v1/client/requests/:id/withdraw`; roll out the API before the updated mobile app. No new environment variable or migration is required. New references begin KCCA-; historical references remain unchanged. Saved maps do not request location permission or use server geocoding. Provider pending data is reconciled through REST on entry, detail return, foreground resume and Refresh Requests; no polling or push guarantee is implied. `test/client_request_test.dart` includes edit/withdrawal regressions and management renders; fake-map renders do not verify native tile behavior.
